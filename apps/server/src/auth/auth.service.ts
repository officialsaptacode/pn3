...
export class AuthService {
  ...
  async signup(dto: RegisterDto) {
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        const prError = error as Prisma.PrismaClientKnownRequestError;
        if (prError.code === "P2002") {
  ...
  async signToken(userId: number, email: string, role: string, userName: string): Promise<Tokens> {
    const payload = {
      sub: userId,
      userName,
      email,

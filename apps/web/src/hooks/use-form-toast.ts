...
function findFirstError<TFieldValues extends FieldValues>(
  ...
  for (const [key, error] of Object.entries(errors)) {
    ...
    if (error.message) return { field: fieldName, message: String(error.message) };
  }
  return null;
}
...
function collectErrorMessages<TFieldValues extends FieldValues>(
  ...
  for (const [key, error] of Object.entries(errors)) {
    ...
    if (!error) continue;
    if (typeof error === "object" && !error.message) {
      messages.push(...collectErrorMessages(error as FieldErrors<TFieldValues>, formatMessage, fieldName));
...
function getLabel(fieldName: string, labels: Record<string, string>): string {
  ...
  return labels[fieldName] || (baseName && labels[baseName]) || fieldName.replace(/([A-Z])/g, " $1").replace(/\./g, " → ").replace(/^./, (str) => str.toUpperCase());
}
...
export function useFormToast<TFieldValues extends FieldValues = Record<string, unknown>>(
  ...
  const showFieldError = useCallback(
    (fieldName: keyof TFieldValues, message: string) => {
      toast.error("Validation Error", { description: formatError(String(fieldName), message, labels), duration });
    },
    [labels, duration]
  );
  ...
  const onError = useCallback(
    (errors: FieldErrors<TFieldValues>) => {
      ...
      if (firstError && autoFocus) setFocus(firstError.field as Path<TFieldValues>, { shouldSelect: true });
      const errorMessages = collectErrorMessages(errors, (f, m) => formatError(f, m, labels));
      if (errorMessages.length === 1 && errorMessages[0]) {
        toast.error("Validation Error", { description: errorMessages[0].replace(/^•\s*/, ""), duration });
      } else {
        const displayed = errorMessages.slice(0, 3).join("\n");
        const more = errorMessages.length > 3 ? `\n...and ${errorMessages.length - 3} more` : "";
        toast.error("Please fix the following errors", { description: `${displayed}${more}`, duration });
      }
    },

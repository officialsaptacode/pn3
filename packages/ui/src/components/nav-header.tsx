...
import type {
  ...
  TeamItem,
  TopicItem,
} from "@workspace/ui/components/types";
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          <CommandGroup heading="Favorites">
            {data.navCollapsible.favorites.map((item: FavoriteItem) => (
              <CommandItem
                className="py-2!"
                key={item.id}
                onSelect={() => {
          ...
          <CommandGroup heading="Teams">
            {data.navCollapsible.teams.map((item: TeamItem) => (
              <CommandItem
                className="py-2!"
                key={item.id}
                onSelect={() => {
          ...
          <CommandGroup heading="Topics">
            {data.navCollapsible.topics.map((item: TopicItem) => (
              <CommandItem
                className="py-2!"
                key={item.id}
                onSelect={() => {
